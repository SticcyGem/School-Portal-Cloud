...
-- REFERENCE DATA (Colleges, Courses, School Years)
-- -----------------------------------------------------------------------------
...
INSERT
INTO academic_terms (term_name, term_start_date, term_end_date, enrollment_start_date, enrollment_end_date, sy_no,
                     term_type_id)
SELECT 'First Semester 2025-2026',
       '2025-08-24',
       '2025-12-18',
       '2025-08-01',
       '2025-12-31',
       ...
       '1S'
FROM sy
WHERE NOT EXISTS (SELECT 1 FROM academic_terms WHERE term_name = 'First Semester 2025-2026');

-- BLOCKS
-- -----------------------------------------------------------------------------
INSERT INTO blocks (course_code, year_level, block_number)
SELECT 'BSCS', 2, 2
WHERE NOT EXISTS (SELECT 1 FROM blocks WHERE course_code = 'BSCS' AND year_level = 2 AND block_number = 2);

INSERT INTO blocks (course_code, year_level, block_number)
SELECT 'BSCS', 3, 2
WHERE NOT EXISTS (SELECT 1 FROM blocks WHERE course_code = 'BSCS' AND year_level = 3 AND block_number = 2);

-- LOCATIONS
-- -----------------------------------------------------------------------------
INSERT INTO buildings (building_name)
VALUES ('Gusaling Villegas')
ON CONFLICT DO NOTHING;
...
INSERT
INTO rooms (room_name, room_type, building_no)
VALUES ('COMP LAB 3', 'LAB', (SELECT building_no FROM b_grounds)),
...
VALUES ('CSC 0212', 'Object Oriented Programming (Lecture)', 2, 0),
...
$$
    ...
    BEGIN
        ...
        IF v_acc_id IS NULL THEN
            ...
            INSERT INTO students (account_id, student_status, education_level, student_type, year_level, course_code,
                                  block_no)
            VALUES (v_acc_id, 'ENROLLED', 'UNDERGRADUATE', 'IRREGULAR', 2, 'BSCS',
                    (SELECT block_no
                     FROM blocks
                     WHERE course_code = 'BSCS'
                       AND year_level = 2
                       AND block_number = 2));
        END IF;
    END
...
-- Dummy Employee (Professor): James Bonifacio
DO
$$
    DECLARE
        v_acc_id UUID;
    BEGIN
        ...
        IF v_acc_id IS NULL THEN
            ...
            -- employee_id auto-generated by trigger
            INSERT INTO employees (account_id, employee_type)
...
$$
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0212' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'ONLINE', 'CSC 0212', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;

            INSERT INTO section_blocks (section_no, block_no)
            SELECT v_sec_no, block_no
            FROM blocks
            WHERE course_code = 'BSCS'
              AND year_level = 2
              AND block_number = 2;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0212.1' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'FACE_TO_FACE', 'CSC 0212.1', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0213' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'ONLINE', 'CSC 0213', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0213.1' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'FACE_TO_FACE', 'CSC 0213.1', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0224' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'ONLINE', 'CSC 0224', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0312' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'ONLINE', 'CSC 0312', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'CSC 0312.1' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'FACE_TO_FACE', 'CSC 0312.1', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'ICC 0105' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'FACE_TO_FACE', 'ICC 0105', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
        ...
        IF NOT EXISTS (SELECT 1 FROM sections WHERE subject_code = 'ICC 0105.1' AND academic_term_no = v_term_no) THEN
            INSERT INTO sections (available_slots, delivery_mode, subject_code, employee_id, academic_term_no)
            VALUES (40, 'FACE_TO_FACE', 'ICC 0105.1', v_emp_id, v_term_no)
            RETURNING section_no INTO v_sec_no;
...
-- ENROLLMENT TRANSACTION (For Joshu Ramos)
-- -----------------------------------------------------------------------------
DO
$$
    DECLARE
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM enrollments WHERE account_id = v_student_acc AND academic_term_no = v_term_no) THEN
            ...
            WHERE s.academic_term_no = v_term_no
              AND s.subject_code IN ('CSC 0212', 'CSC 0212.1', 'CSC 0213', 'CSC 0213.1', 'CSC 0224', 'CSC 0312',
                                     'CSC 0312.1', 'ICC 0105', 'ICC 0105.1');
        END IF;
    END
$$;
